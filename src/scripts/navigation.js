...
router.on('/playlist/:id', async (match) => {
  const { id } = match.data
  lastMatch = match

  const response = await fetch('/fragments/playlist/' + id)
  ...
  const card = findCardByPath('/playlist/' + id)
  let persistentEl
...
router.on('/', async (match) => {
  ...
  document.startViewTransition(() => {
    ...
    const card = findCardByPath(`/${lastMatch?.url}`)
